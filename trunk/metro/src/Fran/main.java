...
public class main {
	...
	private static void dijkstra(Graph<ElementoDecorado<Estacion>, ElementoDecorado<Tramo>> grafo) {
		...
		Estacion est_destino = new Estacion("Gregorio Mara帽on");
		...
		while (iter.hasNext()&& n1_existe==false) {
			...
			// Comprobamos si ya existen los v茅rtices
			aux = iter.next();

			// Si la estaci贸n origen existe.
		...
		///%%%% PORQUE SE SUPONE QUE ES EL PESO DE LA ARISTA, ES DECIR LA DURACI覰 PERO HE PROBADO
	...
	private static void mostrarGrafo(
		...
		while(iter.hasNext()){
			//Vamos pasando de estaci贸n en estacion y mostramos los v茅rtices adyacentes
	...
	private static Graph<ElementoDecorado<Estacion>, ElementoDecorado<Tramo>> construyeGrafo(
		...
		// Declaraci贸n de objetos y variables
		Iterator<Vertex<ElementoDecorado<Estacion>>> iter = null;
